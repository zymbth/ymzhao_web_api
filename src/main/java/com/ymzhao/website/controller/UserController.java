...
public class UserController {
    ...
    public APIResult<User> query(@PathVariable int id) {
    ...
    public APIResult<IPage<User>> getAllUsers(@RequestParam(required = false, defaultValue = "1") Integer page,
                                   @RequestParam(required = false, defaultValue = "20") Integer pagesize) {
        IPage<User> userIPage = iUserService.getAllUsers(page, pagesize);
    ...
    public APIResult<IPage<User>> getAllUsers(@RequestParam(required = false, defaultValue = "1") Integer page,
        ...
        IPage<User> userIPage = iUserService.getUsersByType(page, pagesize, id);
        return APIResult.success(userIPage);
    }

    @ApiOperation(value = "用户登录验证")
    @RequestMapping(value = "login", method = RequestMethod.POST)
    public APIResult<Boolean> query(@RequestParam String usrname, @RequestParam String pwd) {
        Boolean ck = iUserService.login(usrname, pwd);
        return APIResult.success(ck);
    }
}
